      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <aside className="medic-card sticky top-24 overflow-hidden">
            ...
            <div className="p-6"><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm font-semibold text-navy-950">{ar ? 'طلب آمن' : 'Demande sécurisée'}</p><p className="mt-1 text-xs leading-5 text-slate-500">{ar ? 'سيتم تأكيد الموعد من قبل الطبيب.' : 'Le rendez-vous sera confirmé par le médecin.'}</p></div><Link href={actor ? bookPath : `/login/patient?next=${bookPath}`} className="mt-5 flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white hover:bg-brand-700">{t('common.book')}<ArrowRight className="h-4 w-4 rtl:rotate-180" /></Link></div>
          </aside>
        </div>
      </div>
    </article>
